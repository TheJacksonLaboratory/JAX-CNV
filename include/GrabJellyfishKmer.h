#ifndef _GrabJellyfishKmer_H_
#define _GrabJellyfishKmer_H_

#include <getopt.h>
#include <string>


struct SGrabJellyfishKmerCml {
	SGrabJellyfishKmerCml(){}
	SGrabJellyfishKmerCml(const int argc, char** const argv){Parse(argc, argv);}
...
class GrabJellyfishKmer {
 public:
	// Constructors
	GrabJellyfishKmer();
	GrabJellyfishKmer(int argc, char** argv);
	GrabJellyfishKmer(const char * pInput_jfdb, const char * pInput_fasta, const char * pOutput = NULL,
	...
	void SetParameters(const SGrabJellyfishKmerCml & cml);
	void SetParameters(const char * pInput_jfdb, const char * pInput_fasta, const char * pOutput = NULL,
				const char * pRegion = NULL, const int input_bin = 1, const bool input_ascii = false, 
				const bool input_rle = false, const bool input_contig = false);
 private:
	SGrabJellyfishKmerCml cmdline;
	// Not allow to use copy and assign constructors.
	GrabJellyfishKmer(const GrabJellyfishKmer&);
	GrabJellyfishKmer& operator= (const GrabJellyfishKmer&);
};
#endif
